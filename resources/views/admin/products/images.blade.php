...
@section('content')
    <div class="max-w-7xl mx-auto p-6 bg-white dark:bg-gray-800 shadow-lg rounded-lg">
        ...
        <div class="mb-6 flex justify-between items-center">
            ...
            <form action="{{ route('admin.products.upload-images.store', $product->id) }}" method="POST"
                enctype="multipart/form-data" id="uploadForm">
                @csrf
                <input type="file" name="new_image[]" id="new_image" accept="image/*" multiple style="display:none;"
                ...
                <div
                    ...
                    <div class="p-4 bg-white dark:bg-gray-800 rounded-b-lg">
                        ...
                        <div class="flex justify-between items-center space-x-4">
                            @if (!$image->is_primary)
                                <form
                                    action="{{ route('admin.products.update-primary-image', [$product->id, $image->id]) }}"
                                    method="POST">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit"
                            ...
                            <form action="{{ route('admin.products.destroy-image', [$product->id, $image->id]) }}"
                                method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
